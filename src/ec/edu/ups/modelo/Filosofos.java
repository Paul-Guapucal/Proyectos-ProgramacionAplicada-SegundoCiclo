...
public class Filosofos extends Thread {
     ...
     */
    public Filosofos(int id, Tenedor dcha, Tenedor izqda, Portero_del_Comedor comensal, JLabel jLabel_F, 
    ...
    /**
     * Método que se ejecuta indefinidamente, una por cada hilo creado en {@link ControladorPrincipal}
     */
    @Override
    public void run(){
        while(true){ // Se repite infinitamente While
            ...
            try { // try / catch
                ...
                if (!izqda.cogerTenedorIzqdo(id, log)){
                    ...
                    if (ControladoresGenerales.getjTextArea_Log()!=null) log.escribirLog(" El Filósofo " + (id+1) + " tendrá que soltar el tenedor " + (id+1) + " por exceso de tiempo y salir a pensar.");
                    ...
                    if (ControladoresGenerales.getjTextArea_Log()!=null) log.escribirLog(" El Filósofo " + (id+1) + " está pensando.");
                    try {
                        // El tiempo que tarda el filósofo en pensar, entre 100 y 1000 milisegundos:
                        Filosofos.sleep(random.nextInt(1000) + 100);
                    } catch (InterruptedException ex) {
                        System.out.println("Error. Descripción: " + ex.toString());
                        if (ControladoresGenerales.getjTextArea_Log()!=null) log.escribirLog("\n\n Error. Descripción: " + ex.toString() + "\n\n");
                    }
                    ...
                    jTextField_C.setText(" " + (++ControladoresGenerales.filoCount[id])); // Su contador de comidas incrementa una unidad.
                    System.out.println("El Filósofo " + (id+1) + " está comiendo.");
                    if (ControladoresGenerales.getjTextArea_Log()!=null) log.escribirLog(" El Filósofo " + (id+1) + " está comiendo.");
        ...
        try {
            if (ControladoresGenerales.getjTextArea_Log()!=null) log.escribirLog(" La cena ha terminado para todos: El Filósofo " + (id+1) +" se ha puesto a pensar.\n\n Pulsar Iniciar para continuar.\n\n");
        } catch (InterruptedException ex) {
